...
def get_f1(date, suffix):
    ...
    fn = f'/mnt/SSD5/lawrence/VAE/res/{date}/{suffix}/auc_kfold.xlsx'
    ...
    #df_ratio.to_csv(f"/mnt/SSD5/lawrence/VAE/fig_code/RF_predict_{date}_{suffix}.csv", index=False)
...
def draw_new(df, ax, baseline_auc_col = "r_spike", wanted_col="r_pred", name = "AUC mpHFO"):
    ...
    #ax.set_title(f"{name} vs. Num Samples in Arifact Rejection and mpHFO Detection", fontsize=frontsize)
    # set xticks
...
date = "2024-07-01_2"
fn = f"fig_code/RF_predict_all_{date}.csv"
folder = f"./res/{date}"
...
plt.tight_layout()
plt.savefig("./fig/ablation_metrics.jpg", dpi=300)