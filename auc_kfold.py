...
def aggregate(folder, save_suffix ,fold, threshold = 0.5,sampling="GMM"):
    if sampling == "GMM":
        test_fn = f"{folder}/fold_{fold}/{save_suffix}/test_overall_.npz"
        train_fn = f"{folder}/fold_{fold}/{save_suffix}/train_overall_.npz"
    else:
...
def patient_filter(folder, save_suffix, sampling="GMM"):
    ...
    auc_kfold_df = pd.concat(auc_kfold_df)
    auc_kfold_df.to_csv(f"{save_folder}/auc_kfold.csv", index=False)
    # to excel
    writer = pd.ExcelWriter(f"{save_folder}/auc_kfold.xlsx", engine='xlsxwriter')
    for fold in range(len(glob.glob(f"{folder}/fold_*"))):
        train_list_dict[fold].to_excel(writer, sheet_name=f"train_{fold}")
        test_list_dict[fold].to_excel(writer, sheet_name=f"test_{fold}")
    writer.close()
